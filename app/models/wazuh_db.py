...
from ..schemas.wazuh import Agent as AgentSchema
...
class AgentModel:
    def __init__(self, agent: AgentSchema, username: str):
    ...
    @staticmethod
    @handle_es_exceptions
    async def load_from_elasticsearch(agent_id: str) -> Dict:
        query = {
            "query": {
                "bool": {
                    "must": [
        ...
        raise NotFoundUserError(f"Agent with ID {agent_id} not found", 404)

    @staticmethod
    @handle_es_exceptions
    async def load_agents_by_group(group: str) -> List[Dict]:
        query = {
            "query": {
                "bool": {
                    "must": [
...
class EventModel:
    def __init__(self, event: EventSchema, username: str):
